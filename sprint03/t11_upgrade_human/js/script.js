...
class Human {
  ...
  sleepFor() {
    let ask  = +prompt(`Сколько секунд будет спать?`,7);
...
class Superhero extends Human {
  ...
  fly() {
    ...
    function countdown(){
      ...
      modalTotal.innerHTML = `I'm flying! ${fghtTime} &#129464`;
      fghtTime --;
      if (fghtTime <= 0){
        modalTotal.style.display = "none";
        buts.style.opacity = 1;
...
wrap.appendChild(content);
  ...
  flyBtn = document.querySelector('#flyButt'),
  fightBtn = document.querySelector('#fightButt');

//------------- Окно голода в первые 2с
  let openHungry = setTimeout(() => modal.style.display = "block",2000)
// ---------------- Нажатие кнопок
